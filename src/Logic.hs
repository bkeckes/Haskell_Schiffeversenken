...
--fŸgt einen Status im Spielfeld ein. Diese Methode kann sowohl fŸr das gegnerische als auch fŸr das eigene Speilfeld aufgerufen werden
insertStatus::Status->EnemyField->Coord->EnemyField
insertStatus state field (c,i) = M.insert (c,i) state field

--Diese Methode fŸgt von den Koordinaten (x1,y1) bis (x2,y2) den Status state in das Spielfeld ein
...
--Wird von insertStatuus aufgerufen und fŸgt rekursiv den Status in die Koordinaten der Liste Coords ein                 
...
--FŸr jedes Schiff aus MyShips wirs shootShip aufgerufen
